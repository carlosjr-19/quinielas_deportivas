...
const QuinielaDetallePage = () => {
  ...
  const urlInvitacion = `${window.location.origin}/invite/${quiniela?.codigo_acceso}`;

  const copiarEnlaceInv = () => {
    navigator.clipboard.writeText(urlInvitacion)
      .then(() => alert("¡Enlace de invitación copiado!"))
      .catch(() => alert("Error al copiar enlace"));
  };

  const compartirWhatsappInv = () => {
    const texto = `¡Hola! Te invito a unirte a mi quiniela deportiva "${quiniela.nombre}".\n\nIngresa aquí para participar: ${urlInvitacion}`;
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-6">
          <div className="flex flex-col md:flex-row justify-between items-start md:items-center">
            <div className="w-full md:w-auto overflow-hidden">
              ...
              <div className="text-gray-500 mt-2 flex items-center gap-3 flex-wrap">
                <span>Código de Acceso: <span className="font-mono bg-gray-200 px-2 py-1 rounded text-black break-all">{quiniela.codigo_acceso}</span></span>
                <div className="flex gap-2">
                  <button onClick={copiarEnlaceInv} title="Copiar enlace de invitación" className="text-gray-500 hover:text-blue-600 transition-colors p-1 bg-gray-100 rounded hover:bg-blue-50 border border-gray-200">
